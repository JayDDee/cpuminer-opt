...
} x16s_4way_ctx_holder;

x16s_4way_ctx_holder x16s_4way_ctx __attribute__ ((aligned (64)));

// Cube needs one full init so fast reinits can be done in the hash loop.
void init_x16s_4way_ctx()
{
   cubehashInit( &x16s_4way_ctx.cube, 512, 16, 32 );
};


void x16s_4way_hash( void* output, const void* input )
{
...
//   uint32_t inp3[24] __attribute__ ((aligned (64)));
   ...
   if ( s_ntime == UINT32_MAX )
   {
      ...
      x16s_getAlgoString( &tmp[4], hashOrder );
   }
