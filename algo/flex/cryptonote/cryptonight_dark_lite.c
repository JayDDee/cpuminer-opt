...
static void do_dark_lite_groestl_hash(const void* input, size_t len, char* output) {
...
struct flex_cryptonightdarklite_ctx {
...
#if defined(_MSC_VER)
    struct flex_cryptonightdarklite_ctx *ctx = _malloca(sizeof(struct flex_cryptonightdarklite_ctx));
#else
    struct flex_cryptonightdarklite_ctx *ctx = alloca(sizeof(struct flex_cryptonightdarklite_ctx));
#endif
...
void flex_cryptonightdarklite_fast_hash(const char* input, char* output, uint32_t len) {
