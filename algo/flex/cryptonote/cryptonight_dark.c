...
static void do_dark_groestl_hash(const void* input, size_t len, char* output) {
...
struct flex_cryptonightdark_ctx {
...
#if defined(_MSC_VER)
    struct flex_cryptonightdark_ctx *ctx = _malloca(sizeof(struct flex_cryptonightdark_ctx));
#else
    struct flex_cryptonightdark_ctx *ctx = alloca(sizeof(struct flex_cryptonightdark_ctx));
#endif
...
void flex_cryptonightdark_fast_hash(const char* input, char* output, uint32_t len) {
