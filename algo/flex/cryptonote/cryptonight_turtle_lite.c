...
#include "../../gr/cryptonote/crypto/c_blake256.h"
#include "../../gr/cryptonote/crypto/c_skein.h"
#include "../../gr/cryptonote/crypto/int-util.h"
#include "../../gr/cryptonote/crypto/hash-ops.h"
#include "../../gr/cryptonote/crypto/variant2_int_sqrt.h"
...
static void do_turtle_lite_groestl_hash(const void* input, size_t len, char* output) {
    groestl(input, len * 8, (uint8_t*)output);
}

static void do_turtle_lite_skein_hash(const void* input, size_t len, char* output) {
    int r = c_skein_hash(512, input, 8 * len, (uint8_t*)output);
...
struct flex_cryptonightturtlelite_ctx {
...
#if defined(_MSC_VER)
    struct flex_cryptonightturtlelite_ctx *ctx = _malloca(sizeof(struct flex_cryptonightturtlelite_ctx));
#else
    struct flex_cryptonightturtlelite_ctx *ctx = alloca(sizeof(struct flex_cryptonightturtlelite_ctx));
#endif
...
void flex_cryptonightturtlelite_fast_hash(const char* input, char* output, uint32_t len) {
