...
static void do_turtle_groestl_hash(const void* input, size_t len, char* output) {
...
struct flex_cryptonightturtle_ctx {
...
#if defined(_MSC_VER)
    struct flex_cryptonightturtle_ctx *ctx = _malloca(sizeof(struct flex_cryptonightturtle_ctx));
#else
    struct flex_cryptonightturtle_ctx *ctx = alloca(sizeof(struct flex_cryptonightturtle_ctx));
#endif
...
void flex_cryptonightturtle_fast_hash(const char* input, char* output, uint32_t len) {
