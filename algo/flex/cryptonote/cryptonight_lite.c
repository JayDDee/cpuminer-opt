...
static void do_lite_groestl_hash(const void* input, size_t len, char* output) {
...
struct flex_cryptonightlite_ctx {
...
#if defined(_MSC_VER)
    struct flex_cryptonightlite_ctx *ctx = _malloca(sizeof(struct flex_cryptonightlite_ctx));
#else
    struct flex_cryptonightlite_ctx *ctx = alloca(sizeof(struct flex_cryptonightlite_ctx));
#endif
...
void flex_cryptonightlite_fast_hash(const char* input, char* output, uint32_t len) {
