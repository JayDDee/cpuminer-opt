...
/* Software AES, but the header still types u128 as a vector, so it needs the
 * same SIMD target as the rest of the set; compiles to nothing without it. */
...
/* EDIT: tweak_constants() and the haraka_S sponge below are SPHINCS-only and
 * unused by VerusHash. haraka_S_absorb writes t[32] on a 32-byte buffer, which
 * GCC reports as undefined behaviour on every build, so exclude the dead code
